...
class StreamManager:
    ...
    async def start_stream(self, camera_id: str, rtsp_url: str) -> str:
        """
        Start (or refresh) a stream for *camera_id*.
        Returns the HLS playlist path, e.g. /hls/abc123/stream.m3u8
        """
        async with self._lock:
    ...
    def touch_activity(self, camera_id: str) -> None:
    ...
    async def _wait_ready(
        self, camera_id: str, timeout: float = None
    ) -> None:
        timeout = timeout or settings.stream_start_wait_seconds
        m3u8 = Path(settings.hls_dir) / str(camera_id) / "stream.m3u8"
        ...
        while asyncio.get_event_loop().time() < deadline:
            ...
            if stream.process.poll() is not None:
                ...
                logger.error(
                    "FFmpeg for camera %s exited early. stderr: %s",
                    camera_id,
            ...
            if m3u8.exists() and m3u8.stat().st_size > 0:
                ...
                logger.info("Stream %s ready", camera_id)
                return
    ...
    async def _terminate(self, camera_id: str) -> None:
        ...
        logger.info("Stream %s stopped and HLS files cleaned up", camera_id)

    def _hls_url(self, camera_id: str) -> str:
        return f"/hls/{camera_id}/stream.m3u8"
    ...
    async def _check_timeouts(self) -> None:
        ...
        for cam_id, stream in self._streams.items():
            ...
            if idle_seconds > settings.stream_timeout_seconds:
                logger.info(
                    "Stream %s idle for %.0f s — stopping", cam_id, idle_seconds
                )
                to_stop.append(cam_id)
            elif stream.process.poll() is not None:
                logger.warning("Stream %s process died unexpectedly", cam_id)
                to_stop.append(cam_id)
